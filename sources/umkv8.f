...
C Irina update 9/30/2002
     ...
     * 175,225,275,325,375,425,475,525,575/
     ...
     5 SUMSOL,R,PI,WTX,OMFAC,DPSL,VARX,AN,LYR,
...
c SZA dependent noise (Arosa 51 and 101)
  ...
  150 CONTINUE                                                           00000510
      ...
      CALL RDSPECT()                                                     C READ spectral parameters
      CALL READFG()                                                      C READ FG, READ Total ozone at pressure levels 0.5, 0.8, 0.7, 0.9
      CALL RDMSCR()                                                      C READ MULTIPLE SCATTERING CORRECTIONS
      CALL RDNRL()                                                       C Read in NRL temperature/altitude climatology for monthly mean and zonal
      CALL RDREFR()                                                      C READ REFRACTION CORRECTIONS
...
C ***** CHECK FOR END OF BATCH FOR THIS STATION *********************
      ...
      IF (ISTN.NE.ISN) THEN                                             TEMP****
         ...
         CALL CONSOLEMSG('ISTN does not equal Staion Number. Sync lost')
         GOTO 9999                                                          TEMP****
      ENDIF                                                             TEMP****
...
C----------------------------------------------------------------------
      ...
      IMPLICIT NONE
      REAL alfaw(202), acoftw(202), acoftsw(202), betaw(202),
     ...
     2 alftw(61,202),betagw(61,202), sfalfbar(2), sfalft(61,2)
      ...
      COMMON /SPECTRAL/ alfaw, acoftw, acoftsw, betaw,
     1  sfalfa, sfacoft, sfacofts, sfbeta,VALNTC,
     1 sfw, etfw, alamw, rhow, sfbetag,
     2 alftw,betagw, sfalfbar, sfalft

      REAL SUMA,SUMT,SUMTT,SUMB,SUMSL,SUMSH
      INTEGER NR1,NR11,NR2,NR21,NR12,II,I
...
C READ Total ozone at pressure levels 0.5, 0.8, 0.7, 0.9
      ...
      REAL TOTOZTP(21),TABFG(61,21),PRES(61)
      COMMON /FG_IO/TOTOZTP,TABFG,PRES
...
C----------------------------------------------------------------------
      ...
      REAL CQMS(12,42), SLANT(12,1891), TABJMS(61,13,21)
      COMMON /BBBB/ SLANT,CQMS, TABJMS
      INTEGER I,J,K
