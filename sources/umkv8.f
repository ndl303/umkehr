...
C Irina update 9/30/2002
     ...
     * 175,225,275,325,375,425,475,525,575/
     ...
     1 FFF2,FFF1,SUM, TABJ1,TABJ2,TABI1,TABI2,SUMF,OMCHEK,GGG1,
...
C     CALL UMKEHR_OPEN_INPUTFILE(10)                                        ! open (unit=10,file=ft10,status='old'). This is now read from internal buffer '10' generated by decodev4
...
c Correct standard profile for observed TO by adjusting ozone in the lowermost layer
      ...
      if (DXFG(NCP1).lt.0.) then
         ...
         DXFG(NCP1)=1.e-9
         WRITE(OUT6,*)ID(3),ID(4),ID(5),ID(6),OMOBS,sumf,DXFG(NC),
     1                DXFG(NCP1)
         CALL UMKEHR_WRITE_STRING( 6, OUT6)
      end if
