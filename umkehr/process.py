...
def _read_lines_from_input_file( inputfilename : str) ->Tuple[ List[str], Any]:
    ...
    if _is_csv_file(inputfilename):
        try:
            ...
            lines = []
            N14 = None
            print("Error loading level 1 file <", inputfilename, ">. Error = ", err)
    else:
        N14 = None
...
def Level1_to_Level2(  level1inputfilename : str, level2outputfilename : str, KBN=3, verbose=True, covariancetype='R'):
    ...
    if (lines is not None) and (len(lines)>0 ):
        ...
        if (L4 is not None and len(L4) > 0):
            ...
            if verbose: print('No level 2 data records generated by Umkehr algorithm analyzing file {}'.format(level1inputfilename))
    else:
         if verbose: print('No data found in Umkehr Level 1 file {}'.format(level1inputfilename))
